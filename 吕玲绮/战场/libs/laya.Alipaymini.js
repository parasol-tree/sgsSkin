window.aliPayMiniGame = function (exports, Laya) {
	...
	class MiniFileMgr {
	    ...
	    static isNetFile(url) {
	        return (url.indexOf("http://") != -1 || url.indexOf("https://") != -1) && url.indexOf(ALIMiniAdapter.window.my.env.USER_DATA_PATH) == -1;
	    }
	    ...
	    static copyTOCache(tempFilePath, readyUrl, callBack, encoding = "", isAutoClear = true) {
	        ...
	        var totalSize = ALIMiniAdapter.sizeLimit;
	...
	MiniFileMgr.down = window.my.downloadFile;
	...
	class ALIMiniAdapter {
	    ...
	    static init(isPosMsg = false, isSon = false) {
	        ...
	        if (!ALIMiniAdapter.isZiYu) {
	            ...
	            MiniFileMgr.getCacheList(MiniFileMgr.fileNativeDir, Laya.Handler.create(ALIMiniAdapter, ALIMiniAdapter.onMkdirCallBack));
	        }
	        ...
	        if (!ALIMiniAdapter.isZiYu) {
	            ...
	            if (atlasJson) {
	                ...
	                for (i = 0; i < toloadPics.length; i++) {
	                    ...
	                    ALIMiniAdapter.postInfoToContext(Laya.Laya.URL.formatURL(url), Laya.Laya.URL.formatURL(tempAtlasPngUrl), atlasJson);
	                }
	            }
	            else {
